...
class Button:
   ...
   def __init__(self, text, min_width = 200, selected = False):
      if not Button.initialized:
         ...
         button_sheet = pygame.image.load(os.path.join('img','menu_button.png')).convert_alpha()
   ...
   def make_button(text_surface, min_width, button_shape):
      ...
      button.blit(button_shape[0].subsurface(0,0,btrect.width+width,bhrect.height), (0,0))
...
def show_menu(screen, FPS):
   ...
   menu_title = write("Treasure Hunters Menu", 40, (84,183,215), True, "couriernew")
   ...
   while menu:
               ...
               if selected > 2:
                  ...
                  if button_keys[selected] == 'game_sounds':
                     if selections[button_keys[selected]]:
                        button_list[selected].set_label("Game Sounds: On")
                     else:
                        button_list[selected].set_label("Game Sounds: Off")
      ...
      height = button_list[len(button_list)-1].pos[1]-button_list[0].pos[1]+button_list[len(button_list)-1].height+100
